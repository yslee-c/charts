...
---
name: futures-trend
description: 当用户想分析某个期货品种的走势、判断趋势是否可能反转（123 法则 / 2B 法则）时使用。支持中文品种名（如 沪锡、螺纹钢）或代码（如 SN0、rb2410）。
kind: native
parameters:
  type: object
  properties:
    symbol:
      type: string
      description: 期货品种中文名或代码，如 沪锡 / SN0 / 螺纹钢 / rb2410
    bars:
      type: integer
...
被调用后你会收到一段 **JSON** 结果，其中 `summary` 字段是要点摘要，
`trend`/`pivots`/`bearish`/`bullish`/`stops`/`notes` 是结构化依据。请：
1. 主要依据 `summary` 与结构化字段，用简洁中文说明当前趋势、最近摆动高/低点。
2. 说明 123 法则满足了哪几条（① 趋势线 ② 未创新高/低 ③ 破前低/前高）、是否出现 2B。
3. 给出结论与参考止损位；若 `notes` 有跳空提示，请一并告知。
